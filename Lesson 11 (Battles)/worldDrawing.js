...
var possiblePokemonSpawn = [new Bulbasaur()]
for(var i=0; i<3; i++){
    possiblePokemonSpawn.push(false)
}
var HighGrass = function(x, y, i) {
    ...
    this.pokemonHiding = new Bulbasaur()
    this.update = function(){
        ...
        && checkCollision(player.x, player.y, 30, 30, this.x+globalPos[1], this.y+globalPos[0], this.width, this.height)){
            console.log("Hidden " + this.pokemonHiding.name)
            worldSprite[this.index].x = -10000000
            startBattle(this.pokemonHiding)
        }
    }
}